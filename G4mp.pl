/*
MINIMAL PROPOSITIONAL LOGIC PROVER
*/
%%%
...
prover :-
	...
	writex('and modified by Joseph Vidal-Rosset (joseph.vidal.rosset@gmail.com)'),nlx,
	writex('regarding this prover for minimal propositional logic.'),nlx,
%	seq_init,
...
/*
  ...
  * LOGICAL RULES of G4ip prover. 
  ** Non-invertible rules.
  *** Axioms
  **** Identity axiom
    The classical first axiom was in the classical case:
  \begin{verbatim}
   rule([cl,prop], no, ax, S, [], [[]]) :-
	match(S,  ([_X1,[A],_X2]-->[_Y1,[A],_Y2])).
  \end{verbatim}
  This rule is translated in G4ip by a /single conclusion/ sequent.
  It is right to call it  "the identity axiom":
*/
rule([cl,prop], no, ax, S, [], [[]]) :-
	match(S,  ([_X1,[A],_X2]-->[[A]])).
/*
  **** Absurdity axiom
   It translates the famous /Ex Falso Quodlibet/.
  Thanks to the Prolog trick of anonymous variable, i.e.  _  or here _Y, there is no need of Weakening rule.
  This axiom is canceled in the minimal logic prover G4pm. 
*/
/*
rule([cl,prop], no, ax, S, [], [[]]) :-
...
/*
  **** Truth axiom.
  Exactly the converse of the absurdity axiom. 
*/

rule([cl,prop], no, ax, S, [], [[]]) :-
	match(S,  ([_X1,_X2]-->[[top]])).
/*
  **** Rule of negation on the left: formulas move from the right to left.
  
 Note that Only this rule of negation is non-invertible because the inversion means the classical double negation elimination (i.e. the classical /Reductio Ad Absurdum/.
  
*/
rule([cl,prop], no, l(~1), S, [S1], [l(N),r(0)]) :-
	match(S,  ([X1,[(~A)],X2]-->[[bot]])),
	match(S1, ([X1,X2]-->[[A]])),
	length(X1, N).
/*
  **** Rules of disjunction on the right.
  Note that these rules are not invertible in intuitionistic logic, by contrast with the classical case. 

*/
rule([cl,prop], no,  r(\/), S, [S1], [r(N),[r(N),r(N1)]]) :-
	match(S,  ([X1,X2]-->[[(A\/_B)]])),
	match(S1, ([X1,X2]-->[[A]])),
...
rule([cl,prop], no,  l(->), S, [S1, S2], [l(N),r(0),l(N)]) :-
 	match(S,  ([X1,[A->B],X2]-->[Y])),
	match(S1, ([X1,X2]-->[[A]])),
	match(S2, ([X1,[B],X2]-->[Y])),
 	length(X1, N).
/*
  **** Fourth Roy Dyckoff's left rule (hence the label \(Lrd-4\)) : another left rule for conditional on the left.
...
*/
rule([cl,prop], no,  l(rd-4), S, [S1, S2], [l(N),r(0),l(N)]) :-
...
/*
  ** Invertible rules
  *** Roy Dyckhoff's invertible rules
  **** First Roy Dyckhoff's rule on the left (hence the label \(L~rd-1\))
  Rule Lrd-1 is /modus ponens/+ /weakening on the left/; the add of the latter allows invertibility.
...
*/
rule([cl,prop], inv,  l(rd-1), S, [S1], [l(N),r(0),l(N)]) :-
	match(S,  ([X1,[A->B],[A],X2]-->[Y])),
	match(S1, ([X1,[A],[B],X2]-->[Y])),
	length(X1, N).
rule([cl,prop], inv,  l(rd-1), S, [S1], [l(N),r(0),l(N)]) :-
	match(S,  ([X1,[A],[A->B],X2]-->[Y])),
	match(S1, ([X1,[B],[A],X2]-->[Y])),
	length(X1, N).
/*
  **** Second Dyckhoff's rule
 Test  the input
   \begin{verbatim}
   (a/\b) -> c <--> a -> (b -> c).
  \end{verbatim}
  and note the interesting first sequent proof. 
*/
rule([cl,prop], inv,  l(rd-2), S, [S1], [l(N),r(0),l(N)]) :-
	match(S,  ([X1,[((A/\B)->C)],X2]-->[Y])),
...
rule([cl,prop], inv,  l(rd-2), S, [S1], [l(N),r(0),l(N)]) :-
	...
	length(X1, N).
/*
  **** Third Dyckhoff's rule
  This shows the meaning of disjunction when disjunction is on the left of the turnstile.
  This rules allows the automated proof of the following sequent:
  \((p \lor q) \to r \vdash (p \to r) \land (q \to r)\).

*/
rule([cl,prop], inv, l(rd-3), S, [S1], [l(N),r(0),l(N)]) :-
	match(S,  ([X1,[(A\/B)->C],X2]-->[Y])),
	match(S1, ([X1,[(A->C)],[(B->C)],X2]-->[Y])),
	length(X1, N).
rule([cl,prop], inv, l(rd-3), S, [S1], [l(N),r(0),l(N)]) :-
	match(S,  ([X1,[(B\/A)->C],X2]-->[Y])),
	match(S1, ([X1,[(A->C)],[(B->C)],X2]-->[Y])),
	length(X1, N).
rule([cl,prop], inv, l(rd-3-bis), S, [S1], [l(N),r(0),l(N)]) :-
	match(S,  ([X1,[(A\/B)->C],X2]-->[Y])),
	match(S1, ([X1,[(B->C)],[(A->C)],X2]-->[Y])),
	length(X1, N).
/*
  **** Fifth Dyckhoff's rule
  It is a rule  easy to understand: \(\bot \to X\) is reducible to
  \(\top\) and \(\top\) on the left of the turnstile adds no useful
  information for inferences, therefore on can drop \(\bot \to X\)
  when \(\bot \to X\) is on the left of the sequent.
  Rule Lg4ip-5 is useful to give an automated proof of
  the sequent
  \(\neg \neg p \land \neg \bot \vdash \neg (p \to \bot) \) .
  *Remark* : This rule is the second rule of G4ip which has to be deleted to make the prover G4mp: it is /not/ a rule of minimal logic, where \(\bot \to X\) is /not/ reducible to
  \(\top\). 
*/
/*
rule([cl,prop], inv, l(rd-5), S, [S1], [l(N),r(0),l(N)]) :-
	match(S,  ([X1,[bot-> _A],X2]-->[Y])),
	match(S1, ([X1,X2]-->[Y])),
	length(X1, N).
  */
/*
  **** Sixth Dyckhoff's rule
  Reason of this  reduction rule: \(\top \to A\) is reducible to /A/.
*/
rule([cl,prop], inv,  l(rd-6), S, [S1], [l(N),r(0),l(N)]) :-
	match(S,  ([X1,[top->A],X2]-->[Y])),
	match(S1, ([X1,[A],X2]-->[Y])),
	length(X1, N).
/*
 
  *** Usual invertible rules of sequent calculus LJ
  */

/*
  **** Negation definition on the left. 

  */

rule([cl,prop], inv, l(~def), S, [S1], [r(N),l(0)]) :-
	match(S,  ([X1,[~A],X2]-->[Y])),
	match(S1, ([X1,[A->bot],X2]-->[Y])),
	length(X1, N).
  
/*
  **** Rule of negation on the right: formulas move from the left to right. 
  It is the translation of negation introduction in natural deduction. 
 */ 
rule([cl,prop], inv, r(~), S, [S1], [l(N),r(0)]) :-
	match(S,  ([X1,X2]-->[[~A]])),
	match(S1, ([X1,[A],X2]-->[[bot]])),
	length(X1, N).
/*
  **** Negation definition on the right. 
  */


rule([cl,prop], inv, r(~def), S, [S1], [r(N),l(0)]) :-
	match(S,  ([X1,X2]-->[[~A]])),
	match(S1, ([X1,X2]-->[[A->bot]])),
	length(X1, N).
